...
void SwapColumns(int** matrix, int rows, int evenCol, int oddCol, int row = 0) {
    if (row >= rows) {
        return;
    }
    ...
    if (matrix[row][evenCol] == minInEven) {
        matrix[row][evenCol] = maxInOdd;
    }
    if (matrix[row][oddCol] == maxInOdd) {
        matrix[row][oddCol] = minInEven;
    }

    SwapColumns(matrix, rows, evenCol, oddCol, row + 1);
}
